...
type DepartmentRow = DepartmentDto;
...
export default function DepartmentsTable() {
  ...
  const { items, loading, error, refresh, updateDepartment, deleteDepartment } =
    useDepartments({ q: searchValue });

  // ----- Etats des dialogues (par ligne) -----
  const [editDialog, setEditDialog] = useState<DepartmentRow | null>(null);
  ...
  const openEditDialog = (d: DepartmentRow) => {
  ...
  const onSubmitEdit = async (data: { name: string }) => {
    ...
    const ok = await updateDepartment(editDialog.id, { name: data.name });
    if (ok) {
  ...
  const onConfirmDelete = async (d: DepartmentRow) => {
    setIsDeleting(true);
    const ok = await deleteDepartment(d.id);
    if (ok) {
      emitAdminDataRefresh("departments");
    }
    setIsDeleting(false);
  ...
  useEffect(() => {
    const onRefresh = (event: Event) => {
      if (!isAdminEntityRefreshEvent(event, "departments")) return;
      void refresh();
    };
  ...
  const columnsWithActions: ColumnDef<DepartmentRow>[] = columns.map((col) => {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem onClick={() => setConfirmDeleteDialog(d)}>
                Supprimer
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
  ...
  return (
    <div className="space-y-4">
        ...
        >
          ...
          >
            <FieldGroup>
              ...
              />
              <SearchFieldInput placeholder="Rechercher un département" />
              <SearchFieldClear>
      ...
      {loading ? (
        ...
        <Empty className="border border-dashed">
          <EmptyHeader>
            <EmptyTitle>Aucun département</EmptyTitle>
            <EmptyDescription>
              Ajoutez un premier département pour commencer.
            </EmptyDescription>
          </EmptyHeader>
        </Empty>
      ) : (
      ...
      >
        <DialogContent>
          ...
          <p className="text-sm text-muted-foreground">
            ...
            département <strong>{confirmDeleteDialog?.name}</strong> ?
          </p>
