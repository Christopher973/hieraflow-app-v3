...
export default function LocationsAdminPage() {
  ...
  const { createLocation, error } = useLocations();
  ...
  const onSubmit = async (data: FormValues) => {
    const ok = await createLocation({ name: data.name });
    if (ok) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardAction>
          <Button onClick={() => setOpen(true)}>
            <CirclePlus />
            <span className="hidden md:block">Créer une localisation</span>
          </Button>
        </CardAction>
      </CardHeader>
      ...
      <Dialog open={open} onOpenChange={(v) => setOpen(v)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Créer une localisation</DialogTitle>
          </DialogHeader>
